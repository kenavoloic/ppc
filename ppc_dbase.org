...
IF nombre > 0 THEN
SELECT rank() over (order by points desc)  as 'Classement', s.points as 'Score', j.nom as 'Noms', j.prenom as 'Prénoms' from scores s inner join joueurs j on s.joueur = j.joueurId order by s.points desc, j.nom asc limit nombre;
